...
public class UserInputController : MonoBehaviour
{
        ...
        if(_setFollowObject)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(ray, out hit, Mathf.Infinity, layerMask))
                {
                    ...
                    if (!(parent && parent.CompareTag("Enemy") || transform.CompareTag("Medicine cabinet")))
                        transform = null;
                    if (transform)
                    {
                        Debug.Log("attack");
    ...
    void UpdateState()
    {
        if (Input.GetKeyDown(KeyCode.Escape)
            || (Input.GetKeyDown(KeyCode.LeftControl) && !_setAlly))
        {
        ...
        if ((!Input.GetKey(KeyCode.Escape) && Input.GetMouseButtonDown(0) && !_setFollowObject)
            || (!Input.GetKey(KeyCode.LeftControl) && _setFollowObject))
        {
            _cameraController.RotateCamera = true;
            _cameraController.LockCursor = true;
        }

        if (Input.GetKeyDown(KeyCode.LeftAlt) && !_setFollowObject)
            _cameraController.RotateCamera = false;

        if (Input.GetKeyUp(KeyCode.LeftAlt) && _setAlly)
            _cameraController.RotateCamera = true;
        ...
        if (_setAlly && Input.GetKeyUp(KeyCode.LeftAlt))
            _setAlly = false;

        if (Input.GetKeyDown(KeyCode.LeftControl) && !_setAlly)
            _setFollowObject = true;

        if (_setFollowObject && Input.GetKeyUp(KeyCode.LeftControl))
